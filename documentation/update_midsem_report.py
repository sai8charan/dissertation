"""
documentation/update_midsem_report.py
-------------------------------------
Apply faculty-requested updates to MidSem_Report_2024AA05606.docx.

Run from dissertation root:
    python documentation/update_midsem_report.py
"""
...
DOC_PATH = ROOT / "documentation" / "MidSem_Report_2024AA05606.docx"
...
def _remove_existing_chapter2(doc: Document) -> None:
    """Remove a previously inserted or misplaced Chapter 2 block."""
...
def _remove_duplicate_contributions(doc: Document) -> None:
    """Remove all Contributions blocks (re-inserted in correct position)."""
    to_delete = []
    for i, para in enumerate(doc.paragraphs):
        t = para.text.strip()
...
def main() -> None:
    if not DOC_PATH.exists():
        raise FileNotFoundError(f"Report not found: {DOC_PATH}")
